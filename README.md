...
`tools/list` at `https://gateway.pipeworx.io/business-feeds/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/business_feeds_list_feeds \
  -H 'Content-Type: application/json' \
  -d '{"category":"business"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/business_feeds_list_feeds`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "business-feeds": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-business-feeds"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-business-feeds
```
